...
def align_features(targets, features, fps):
    X, Y = [], []
    frame_duration = 1.0 / fps
    dropped = 0
    
    for t_data in targets:
        start_t = t_data["timestamp"]
        end_t = start_t + frame_duration
        
        window_features = [f for f in features if start_t <= f.start_time < end_t]
        if not window_features:
            dropped += 1
            continue
            
        mels = np.array([f.log_mel for f in window_features])
        avg_mel = np.mean(mels, axis=0) # (80,)
...
def build_dataset():
    ...
    # Split Strategy: First 4 -> Train, Next 1 -> Val, Last 1 -> Test
    videos.sort()
    splits = {
        "train": videos[:4],
    ...
    for split_name, files in splits.items():
        for f in files:
            ...
            tmp_audio = os.path.join(out_dir, f"{seq_id}_audio.wav")
            ...
            X_arr, Y_arr, dropped = align_features(targets, features, fps)
    ...
    for k in ["train", "val", "test"]:
        if dataset[k]["X"]:
            ...
            final_dataset[k] = {"X": torch.empty((0, 80)), "Y": torch.empty((0, 4)), "sequences": 0}
            
    dataset_path = os.path.join(out_dir, "dataset_v2.pt")
