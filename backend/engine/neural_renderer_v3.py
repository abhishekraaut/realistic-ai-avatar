...
class NeuralRendererV3(nn.Module):
    def __init__(self, motion_dim=11):
        ...
        self.bottleneck_film = FiLM(256, 256)
        
        # Temporal Module (Causal)
        self.conv_lstm = ConvLSTMCell(input_dim=256, hidden_dim=256, kernel_size=3)
    ...
    def forward(self, identity_ref, motion, state=None):
        ...
        # Modulate bottleneck with current motion before LSTM
        f4_cond = self.bottleneck_film(f4, cond)
        
        if state is None:
            # Zero initialization for temporal state
            B, C, H, W = f4_cond.shape
            state = (torch.zeros(B, self.conv_lstm.hidden_dim, H, W, device=f4_cond.device),
                     torch.zeros(B, self.conv_lstm.hidden_dim, H, W, device=f4_cond.device))
            
        h, c = self.conv_lstm(f4_cond, state)
        
        d = self.dec4(h, f3, cond)
